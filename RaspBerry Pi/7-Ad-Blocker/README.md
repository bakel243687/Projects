...
### DNS Sinkholing
At its core, Pi-hole operates as a **DNS sinkhole**. When a device on the
network requests the IP address of a known malicious or unwanted domain,
Pi-hole intercepts that query and returns a null address, the connection
never leaves the network. Security Operations Centre (SOC) teams deploy
this exact technique to cut off malware from its Command & Control (C2)
infrastructure.

### Cybersecurity Applications

**Malware C2 Blocking**
Infected devices often beacon out to C2 servers for instructions. A DNS
sinkhole blocks that communication at the network level before it ever
reaches the internet, effectively neutralising the malware's ability to
operate.

**Threat Intelligence Integration**
Pi-hole supports custom blocklists. Security-focused blocklists from threat
intelligence sources such as abuse.ch and URLhaus can be imported directly,
turning Pi-hole into a live IOC (Indicator of Compromise) blocking engine
at the DNS layer.

**Rogue Device & Anomaly Detection**
Every DNS query on the network is logged with a timestamp and source IP.
Unusual query patterns, a device querying suspicious domains at odd hours,
or abnormally high query volumes, can indicate a compromised device on the
network.

**DNS Exfiltration Detection**
DNS-based data exfiltration is a real and commonly used attack technique
where attackers tunnel stolen data out of a network disguised as DNS queries.
Full DNS logging, as configured in this setup, is a foundational step toward
detecting this kind of activity.

**Network Visibility**
Security starts with visibility. Pi-hole provides a full, real-time log of
every domain every device on the network is communicating with, a capability
that mirrors what enterprise network monitoring tools offer at a fraction of
the cost.
...
> **Note:** Replace `***REMOVED***` with your Pi's current IP if it differs. Replace `***REMOVED***` with your router's gateway IP if needed. You can confirm it with `ip route | grep default`.
...
1. Open a browser and go to your router's admin page (usually `http://***REMOVED***`)
...
| Queries blocked (first 24hrs) | 157 |
| Percentage blocked | 11.7 |
